    ...
    if not opt.is_eval:
        ...
        for epo in range(start_epoch, opt.epoch + 1):
            ...
            average_error = np.mean(
                (ret_test["#80ms"], ret_test["#160ms"], ret_test["#320ms"], ret_test["#400ms"], ret_test["#560ms"],ret_test["#1000ms"]))
            ...
            err_value = 'AverageError{:.4f}_err{:.4f}_err{:.4f}_err{:.4f}_err{:.4f}_err{:.4f}_err{:.4f}'.format(
                average_error, ret_test["#80ms"], ret_test["#160ms"], ret_test["#320ms"], ret_test["#400ms"], ret_test["#560ms"],ret_test["#1000ms"])
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, p3d_h36 in tqdm(enumerate(data_loader)):
        ...
        p3d_out_all_0,p3d_out_all_1,p3d_out_all,weight = net_pred(p3d_src, input_n=in_n, output_n=10, itera=itera)
        ...
        if is_train == 0:
            ...
            p3d_out_all_0 = p3d_out_all_0.reshape([batch_size, seq_in + out_n, itera, len(dim_used) // 3, 3])
            p3d_out_all_1 = p3d_out_all_1.reshape([batch_size, seq_in + out_n, itera, len(dim_used) // 3, 3])

            loss_p3d = torch.mean(torch.norm(p3d_out_all[:, :, 0] - p3d_sup, dim=3))+\
                torch.torch.mean(torch.norm(p3d_out_all_0[:, :, 0] - p3d_sup, dim=3))+ \
                torch.torch.mean(torch.norm(p3d_out_all_1[:, :, 0] - p3d_sup, dim=3))

            loss_l1 = torch.mean(torch.norm(weight, p=1, dim=0))
            loss_all = loss_p3d + opt.l1norm * loss_l1
    ...
    if is_train <= 1:
        ...
        for j in range(out_n):
            ret["#{:d}ms".format(titles[j]*40)] = m_p3d_h36[j]
    return ret
